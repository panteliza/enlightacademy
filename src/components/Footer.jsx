        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          ...
          <div>
            ...
            <ul className="space-y-2 text-sm">
              {["ACLS BLS Training", "Ventilator Training", "Airway Management Training", "Infection Prevention Training", "Waste Management Training"].map((service, index) => (
                <li key={index}>
                  <Link to={`/${service.toLowerCase().replace(/\s+/g, "-")}`} className="hover:text-green-300 transition-all duration-300">
                    {service}
                  </Link>
                </li>
              ))}
